...
# Custom Font Upload Endpoints
@router.post("/uploads/fonts/presign")
@limiter.limit("10/minute")
async def presign_custom_font(request: Request, payload: Dict[str, Any] | None = None):
    """Create a presigned URL to upload a custom font file to R2."""
    try:
        uid = _verify_firebase_uid(request)
    except Exception:
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    payload = payload or {}
    filename = str(payload.get('filename') or 'font.woff2').lower()
    content_type = str(payload.get('contentType') or 'font/woff2')
    ...
    if filename.endswith('.woff2'):
        ...
        raise HTTPException(status_code=400, detail="Invalid font format. Supported: .woff2, .woff, .ttf, .otf")
    
    # Generate unique key
    font_id = uuid.uuid4().hex
    key = f"fonts/{uid}/{font_id}.{font_format}"
    ...
    try:
        ...
        logger.exception("presign_custom_font failed: %s", e)
        raise HTTPException(status_code=500, detail="Failed to create upload URL")

@router.post("/uploads/fonts/upload")
@limiter.limit("10/minute")
async def upload_custom_font(request: Request):
    """Direct upload endpoint for custom fonts."""
    try:
        uid = _verify_firebase_uid(request)
    except Exception:
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    try:
        ...
        form = await request.form()
        file = form.get('file')
        font_name = form.get('fontName', 'Custom Font')
        ...
        # Determine format
        filename = getattr(file, 'filename', 'font.woff2').lower()
        font_format = 'woff2'
        content_type = 'font/woff2'
        ...
        async with async_session_maker() as session:
            await session.execute(
                text("""
                    INSERT INTO custom_fonts (id, user_id, font_name, font_url, font_format, file_size, created_at, updated_at)
                    VALUES (:id, :uid, :name, :url, :format, :size, NOW(), NOW())
                """),
                {
                    "id": font_id,
                    "uid": uid,
                    "name": str(font_name).strip() or 'Custom Font',
                    "url": public_url,
                    "format": font_format,
                    "size": file_size
                }
        ...
        logger.exception("upload_custom_font failed: %s", e)
        raise HTTPException(status_code=500, detail="Failed to upload font")

@router.get("/fonts")
@limiter.limit("60/minute")
async def list_custom_fonts(request: Request):
    """List all custom fonts uploaded by the user."""
    try:
        uid = _verify_firebase_uid(request)
    except Exception:
        raise HTTPException(status_code=401, detail="Unauthorized")
    ...
    try:
        async with async_session_maker() as session:
            ...
            for row in rows:
                fonts.append({
                    ...
                    "url": row.get("font_url"),
                    "format": row.get("font_format"),
        ...
        logger.exception("list_custom_fonts failed: %s", e)
        raise HTTPException(status_code=500, detail="Failed to list fonts")

@router.delete("/fonts/{font_id}")
@limiter.limit("30/minute")
async def delete_custom_font(request: Request, font_id: str):
    ...
    try:
        ...
        logger.exception("delete_custom_font failed: %s", e)
